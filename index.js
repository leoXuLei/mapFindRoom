...
function initfirstLevelArea() {
  const myGeo = new BMap.Geocoder() // Geocoder类 用于获取用户的地址解析
  let asyncFinishedNum = 0 // 获取一级城区数据异步任务完成的计数
  $.each(cityAreaNameArr, function (index, item) {
    (function (item) {
      const a = {
        id: index,
        name: item,
        count: Utils.random(4, 9), // 套数：随机数
        price: Math.floor(Utils.random(4, 9) * Math.random() * 10) / 10 // 价格：随机数
      }
      // 解析用户输入的地址
      myGeo.getPoint(
        '上海市' + item,
        function (point) {
          ...
          ++asyncFinishedNum
          // 一级城区解析完毕后
          if (asyncFinishedNum === cityAreaNameArr.length) {
            showFirstAreaLabels() // 绘制一级城区标注聚合点
...
function showThreeLevelsByZoom() {
  ...
  if (zoomLevel == 11 || zoomLevel == 12 || zoomLevel == 13) {
    ...
    if (secondCircleLabelList.length > 0) {
      addViewLabel(secondCircleLabelList) // 根据地图视野动态加载数据
    } else {
      showSecondCircleLabels() // 绘制二级商圈标注聚合点
    }
  } else {
    // 展示三级楼盘（当存储的数据有数时直接加载视野数据，否则请求接口数据）
    if (thirdBuildingLabelList.length > 0) {
      addViewLabel(thirdBuildingLabelList) // 根据地图视野动态加载数据
    } else {
      showThirdBuildingLabels() // 绘制三级楼盘标注聚合点
    }
  }
}

//绘制一级城区标注聚合点
function showFirstAreaLabels() {
  ...
  $.each(firstLevelArea, function (index, data) {
    ...
    const myLabel = new BMap.Label(tpl, {
      position: point,
    ...
    myLabel.addEventListener('mouseover', function(event) {
      myLabel.setStyle({
        background: 'rgba(240,65,52,0.9)',  // 覆盖物背景颜色
        zIndex: 4
      }) 
      // 不用var声明是要timer为全局变量，为下面鼠标移出时间清理定时器
      areaLabelTimer = setTimeout(function() {
...
// 绘制二级商圈标注聚合点
function showSecondCircleLabels() {
  map.clearOverlays()
  $.each(secondLevelCircle, function (index, data) {
    ...
    secondCircleLabelList.push(myLabel)
    ...
    myLabel.addEventListener('mouseover', function () {
      ...
      circleLabelTimer = setTimeout(function () {
        //不用var声明是要timer为全局变量，为下面鼠标移出时间清理定时器
  ...
  addViewLabel(secondCircleLabelList)
}

//绘制三级楼盘标注聚合点
function showThirdBuildingLabels() {
  map.clearOverlays()
  window.thirdBuildingLabelList = []
  $.each(thirdLevelBuilding, function (index, data) {
    ...
    myLabel.setTitle(data.name)
    thirdBuildingLabelList.push(myLabel)
  ...
  if (!isInDrawing) {
    addViewLabel(thirdBuildingLabelList) //加载可视范围点
  }
}
...
function bindDrawEvents(map) {
  ...
  UI.drawBtn.addEventListener('click', function (e) {
    ...
    if (window.onlyOnceFlag) {
      showThirdBuildingLabels() //绘制三级楼盘标注聚合点(为了解决：在二级商圈画圈时还没有三级楼盘数据问题)
