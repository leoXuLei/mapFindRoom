...
*/
    ...
    var cityAreas = ["浦东新区","徐汇", "长宁", "黄浦", "奉贤", "金山", "松江", "闵行", "普陀区","青浦", "闸北", "虹口", "杨浦", "嘉定","宝山"]; //城市城区
    // 一级城区假数据
    var areaFakeData=[];            
    // 二级商圈假数据
    var circleFakeData= [
    ...
    var buildingFakeData=[
            {"name":"楼盘一","code":"01","longitude":"121.522006","latitude":"31.239117","num":"100","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘二","code":"02","longitude":"121.526066","latitude":"31.245855","num":"200","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘三","code":"03","longitude":"121.533935","latitude":"31.234632","num":"300","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘四","code":"04","longitude":"121.55068","latitude":"31.225492","num":"400","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘五","code":"05","longitude":"121.563543","latitude":"31.209433","num":"500","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘六","code":"06","longitude":"121.594158","latitude":"31.208012","num":"600","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘七","code":"07","longitude":"121.600805","latitude":"31.216552","num":"700","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘八","code":"08","longitude":"121.603993","latitude":"31.213874","num":"800","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘九","code":"09","longitude":"121.603454","latitude":"31.207975","num":"900","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘十","code":"010","longitude":"121.613479","latitude":"31.207758","num":"1000","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘十一","code":"011","longitude":"121.629253","latitude":"31.220096","num":"1100","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
            {"name":"楼盘十二","code":"012","longitude":"121.627025","latitude":"31.216823","num":"1200","price": `${Math.floor(Utils.random(4, 9) * Math.random()*10)/10}`},
    ];
    ...
    function initFakeData() {
        ...
        var count = 0; // 用于判断获取一级城区数据异步任务是否都完成的计数
        $.each(cityAreas, function(index, item) {
            (function(item) {
                ...
                myGeo.getPoint("上海市" + item, function(point) { // 解析地址
    ...
    // 初始化地图
    function initMap() {
        var map = new BMap.Map("allMap", {enableMapClick: false, minZoom: 11, maxZoom: 19});
	    ...
	    map.addControl(new BMap.NavigationControl({ type: BMAP_NAVIGATION_CONTROL_ZOOM, anchor: BMAP_ANCHOR_BOTTOM_RIGHT, offset: new BMap.Size(20, 20) })) // 右下角，添加放大缩小控件
    ...
    function showThreeLevelsByZoom() {
        ...
        if(zoomLevel==11 || zoomLevel==12 || zoomLevel==13){
            showMapAreas(); // 绘制一级城区聚合点
        }else if (zoomLevel==14 || zoomLevel==15) {

            // 展示二级商圈（当存储的数据有数时直接加载视野数据，否则请求接口数据）
            if(circleLabelList.length>0){
                addViewLabel(circleLabelList);  // 根据地图视野动态加载数据
            }else{
                showMapCircles();            // 绘制二级商圈点位信息
            }
        } else {
            // 展示三级楼盘（当存储的数据有数时直接加载视野数据，否则请求接口数据）
            if(buildingLabelList.length>0){
                addViewLabel(buildingLabelList);  // 根据地图视野动态加载数据
            }else{
                showMapBuildings();            // 绘制三级楼盘点位信息
            }
        }
    }
    ...
    function showMapAreas() {
        ...
        $.each(areaFakeData,function (index,data) {
            ...
            var tpl = '<div class="bubble bubble-1" data-longitude="' + data.longitude + '"' +
                ...
                '<p><span class="count">' + data.price + '</span>万</p>' +
                '<p><span class="count">' + data.count + '</span>套</p>' +
                '</div>';
            ...
            myLabel.addEventListener("mouseover", function (event) {
                    myLabel.setStyle({background:"rgba(240,65,52,0.9)"}); // 修改覆盖物背景颜色
                    areaLabelTimer = setTimeout(function(){ // 不用var声明是要timer为全局变量，为下面鼠标移出时间清理定时器
                        var areaName = myLabel.getTitle();
                        getBoundary(areaName); // 一级城区的多边形边界绘制
                    },100)
            });
            // 当鼠标离开本label时删除多边形边界
            myLabel.addEventListener("mouseout", function (event) {
                    clearTimeout(areaLabelTimer);//清理定时器
                    myLabel.setStyle({background:"#457cf4"}); //修改覆盖物背景颜色
            ...
            myLabel.addEventListener("click", function (result) {
                ...
                map.setZoom(14); // 放大到二级
            });

        })
    }

    // 一级城区的多边形边界绘制（这是通过Boundary，也可以像二级绘制多边形边界那样通过new BMap.Polygon(data.border) 实现 ）
    function getBoundary(areaName) {
        var bdary = new BMap.Boundary();
        bdary.get(areaName, function (rs) {
            var count = rs.boundaries.length; // 行政城区的点有多少个
            if (count === 0) {
                alert('未能获取当前输入行政城区');
                return;
            }
            for (var i = 0; i < count; i++) {
                ...
                window.polygonArr.push(tempply); 
    ...
    function showMapCircles() {
        map.clearOverlays();      
        $.each(circleFakeData,function (index,data) {
            var point = new BMap.Point(data.longitude, data.latitude);
            //  自定义label样式
            var tpl = '<div class="bubble bubble-1" data-longitude="' + data.longitude + '"' +
                ' data-latitude="' + data.latitude + '" data-id="' + data.id + '">' +
            ...
            myLabel.setTitle(data.name);
            circleLabelList.push(myLabel);

            // 当鼠标悬停在本label上时显示多边形边界
            myLabel.addEventListener("mouseover", function () {
                myLabel.setStyle({background:"rgba(240,65,52,0.9)"}); //修改覆盖物背景颜色
                circleLabelTimer = setTimeout(function(){ //不用var声明是要timer为全局变量，为下面鼠标移出时间清理定时器
                    var tempply = new BMap.Polygon(data.border, {
                        strokeWeight: 2,	//	边线的宽度，以像素为单位
                        strokeColor: "rgb(66, 133, 244)", // 边线颜色
                        fillColor: "rgba(53, 126, 244,0.1)" //填充颜色
                    }); //建立多边形覆盖物
            ...
            // 当鼠标离开本label上时删除边界折线数据
            myLabel.addEventListener("mouseout", function () {
                clearTimeout(circleLabelTimer);//清理定时器
                myLabel.setStyle({background:"#457cf4"}); //修改覆盖物背景颜色
                clearAllPolygon(); // 清楚地图上的所有多边形边界
            });
            // 当鼠标点击本label时
            myLabel.addEventListener("click", function (result) {
                map.setCenter(result.point)// 重新设置地图中心点后再放大
                map.setZoom(16); // 放大到三级
            });

        })  
        addViewLabel(circleLabelList);
    }
    ...
    function showMapBuildings() {
        ...
        $.each(buildingFakeData,function (index,data) {
            var point = new BMap.Point(data.longitude, data.latitude);
            //  自定义label样式
            var tplLabel = `<p class="bubble-3 bubble" data-longitude="${data.longitude}"
            data-latitude="${data.latitude}" data-id="${data.id}">
            ...
            var myLabel = new BMap.Label( tplLabel,
                {
                    position: point, // label 在此处添加点位位置信息
            ...
            myLabel.setTitle(data.name);
            buildingLabelList.push(myLabel);
            ...
            //鼠标悬停在本label上时
            myLabel.addEventListener("mouseover", function () {
                myLabel.setStyle({background:"rgb(252, 96, 90)" ,border: "rgb(252, 96, 90)"}); //修改覆盖物背景颜色
                infoBox.open(point);
            });
            // 当鼠标离开本label时
            myLabel.addEventListener("mouseout", function () {
                // 从本label离开时，如果 没有点击过label 或者 如果本次离开的的不是上次点击的label，则将本label置为初始样式，信息窗关闭       
                if (!window.lastMyLabel || window.lastMyLabel.getPosition().lng !== myLabel.getPosition().lng || window.lastMyLabel.getPosition().lat !== myLabel.getPosition().lat) {
                    myLabel.setStyle({background:"#3072f6" ,border: "#3072f6"}); //修改覆盖物背景颜色
                    infoBox.close();
                }
           ...
           //鼠标点击本label时
            myLabel.addEventListener("click", function () {
                ...
                if (window.lastMyLabel && window.lastMyLabel.getPosition().lng !== myLabel.getPosition().lng && window.lastMyLabel.getPosition().lat !== myLabel.getPosition().lat) {
                    window.lastMyLabel.setStyle({background:"#3072f6" ,border: "#3072f6"});
                    lastMyLabel.setContent(tplLabel)
                ...
                myLabel.setStyle({background:"rgb(252, 96, 90)" ,border: "rgb(252, 96, 90)"}); //修改覆盖物背景颜色
                let tplColored = `<p class="bubble-3 bubble" data-longitude="${data.longitude}"
                                    data-latitude="${data.latitude}" data-id="${data.id}">
                                    <span class="num">${data.name}  ${data.price}万  ${data.num}套</span>
                                    <i class="arrow-up arrow-up-colored"></i>
                                  </p>`;
                myLabel.setContent(tplColored)
        ...
        if(!isInDrawing) {
            addViewLabel(buildingLabelList);//加载可视范围点
        }
    }
    ...
    // 清除地图上的所有多边形边界（一级城区、二级商圈）
    function clearAllPolygon () {
        if (window.polygonArr.length) {
            var tempLength = window.polygonArr.length;
            for(var i = 0; i < tempLength; i++ ) {
                if (window.polygonArr[i]) {
                    var polyPathArray = new Array();
                    window.polygonArr[i].setPath(polyPathArray);
                    map.removeOverlay(window.polygonArr[i]);//清除折线数据
                }
            }
            window.polygonArr = [];
        }
    ...
    function bindEvents(map){
        ...
        drawBtn.addEventListener('click',function(e){
            ...
            if(window.onlyOnceFlag) {
                showMapBuildings();            //绘制三级楼盘绘制点位信息(为了解决：在二级商圈画圈时还没有楼盘数据问题)
        ...
        map.addEventListener('mouseup',function(e){
            ...
            if(isInDrawing && isMouseDown){
                ...
                for(var i=0;i<ret.length;i++){
                    ...
                    li.innerText ? li.innerText = ret[i] : li.textContent = ret[i].getTitle();
